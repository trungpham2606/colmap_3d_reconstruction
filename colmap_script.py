...
def extract_and_match_sift(colmap_path, database_path, image_dir):
    cmd = [
        ...
        '--database_path', str(os.path.join(os.path.split(os.path.abspath(__file__))[0],database_path)),
        '--image_path', str(os.path.join(os.path.split(os.path.abspath(__file__))[0], image_dir)),
    ]
    # logging.info(' '.join(cmd))
...
def build_colmap_model_no_pose(database: BaseDatabase, colmap_path='colmap', output_dir=None):
    colmap_root = Path(output_dir) / database.database_name / 'colmap'
    colmap_root.mkdir(exist_ok=True, parents=True)
    if not os.path.exists(colmap_root):
